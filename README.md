...
_Deep Learning Fall 2020, Project Report_

## Combining Vision and Language:

Creating an Image-Captioning Architecture using Deep Learning

_Brian Xu, Nihar Dwivedi, Simon Gilbert, Kate Flanagan, Aakash Sarkar_

_{_[_brianxu_](mailto:brianxu@bu.edu)_,_[_ndwivedi_](mailto:ndwivedi@bu.edu)_,_[_simonsai_](mailto:simonsai@bu.edu)_,_[_kateflan_](mailto:kateflan@bu.edu)_,_[_aakash18_](mailto:aakash18@bu.edu)[_}_](mailto:aakash18@bu.edu)[_@bu.edu_](mailto:aakash18@bu.edu)

![](RackMultipart20201229-4-1tmpif1_html_fce0f366dbd0a7b6.png)
...
State of the art approaches to image captioning have used separate architectures for the image and language processing portions of the task. Typically, CNNs are used for embedding image features and RNNs are favored for embedding the textual components. These embeddings are combined in what has been deemed the &quot;joint embedding&quot; approach [3]. Such an approach is useful for linking linguistic and visual concepts and can also be used for more complicated tasks such as visual question answering [4].

RNNs have been favored for language embeddings due to their ability to preserve sequential relationships. However, in 2017 a new type of neural net known as a transformer was proposed by Vaswani et al. at Google [6]. Transformers are capable of linking features that are relatively distant from each other in the input space and have the advantage of being parallelizable, which allows them to take less time to train while performing at or beyond the level of RNNs [1].

All of this suggests that transformers may be ideal candidates to replace RNN layers in certain applications.
...
![](RackMultipart20201229-4-1tmpif1_html_f5ef12e20c533fda.png)
...
![](RackMultipart20201229-4-1tmpif1_html_95d02f0315cbc2ce.png)
...
![](RackMultipart20201229-4-1tmpif1_html_f7d1e62df7fdb828.png)
...
These two images are sample outputs that we obtained through the Resnet and VGG models. ![](RackMultipart20201229-4-1tmpif1_html_b4113190581f1fe5.png)

![](RackMultipart20201229-4-1tmpif1_html_fa36db32c6fdfd2e.png)
...
These two images are a couple of sample outputs that we obtained from our transformer implementations. ![](RackMultipart20201229-4-1tmpif1_html_dc4d2cc05062d67f.png)

![](RackMultipart20201229-4-1tmpif1_html_af22ce3f9b0e5fa1.png)
...
| **Task** | **File names** | **Who** |
| --- | --- | --- |
| Implemented Resnets, incorporated BLEU into the Resnet, VGG, Transformer models, helped write report | VGG.ipynb, Resnet.ipynb | Brian |
| Implemented VGG, incorporated BLEU into the Resnet, VGG models, helped write the project report, Ported the project to GitHub | VGG.ipynb, Resnet.ipynb | Nihar |
| Implemented Transformers, incorporated BLEU into the transformer model, created the video presentation, helped write report | Transformer.ipynb | Simon |
| Implemented BLEU, initial resnet model, incorporated BLEU into Resnet, created the presentation for the project, helped write reports, general coordination | VGG.ipynb, Resnet.ipynb | Kate |
| Helped set up the Coco dataset and preprocessing and worked on the LSTM decoder module, implemented the CNN + transformer architecture, incorporated BLEU into the transformer model, wrote the transformer section of the project report with Simon | Transformer.ipynb | Aakash |
